...
export const EXERCISE_VISUALS: ExerciseVisual[] = [
  ...
  entry("frappe-volee-ballon-lance", "Frappe de volée sur ballon lancé", M.VOLLEY_STRIKE, "exact", { ball: true }),
  entry("controle-oriente-frappe-rapide", "Contrôle orienté + frappe rapide", M.BALL_CUT_INSIDE, "exact", { ball: true }),
  entry("centre-frappe-couloir", "Centre du couloir + reprise", M.PASS_STRIKE, "exact", { ball: true, cones: true }),
  ...
  entry("finition-1v1-gardien", "Finition en 1 contre 1 face au gardien", M.SHOT_STRIKE, "exact", { ball: true }),
  ...
  entry("remise-une-touche", "Remise en une touche", M.PASS_STRIKE, "exact", { ball: true, cones: true }),
  ...
  entry("defense-1v1-face-dribbleur", "Défense en 1 contre 1 face à un dribbleur", M.LATERAL_SHUFFLE, "exact", { cones: true }),
  ...
  entry("renforcement-mollets-pointes", "Renforcement mollets (montées sur pointes)", M.CALF_RAISES, "exact"),
  ...
  entry("gainage-rotation-medecine", "Gainage rotatif (anti-rotation)", M.ANTI_ROTATION_PRESS, "exact"),
  entry("tirage-elastique-dos", "Tirage élastique pour le dos", M.BAND_ROW, "exact"),
  ...
  entry("sprint-changement-direction-5-10-5", "Sprint + changement de direction", M.CHANGE_OF_DIRECTION_SPRINT, "exact", { cones: true }),
  entry("sauts-a-la-corde", "Sauts à la corde", M.ROPE_JUMP, "exact"),
  entry("departs-reactifs-signal", "Départs réactifs sur signal", M.SPRINT, "exact"),
  entry("montees-marche-explosives", "Montées de marche explosives", M.HIGH_KNEES, "exact"),
  ...
  entry("echelle-agilite-pieds", "Échelle d'agilité", M.HIGH_KNEES, "exact"),
  entry("changement-direction-reactif-partenaire", "Changement de direction réactif", M.LATERAL_SHUFFLE, "exact"),
  entry("bondissements-avant-reception", "Bondissements avant (réception amortie)", M.FORWARD_BOUND, "exact"),

  // CARDIO
  entry("fractionne-30-30", "Course fractionnée 30/30", M.SPRINT, "exact"),
  entry("footing-continu", "Footing continu", M.JOG, "exact"),
  entry("circuit-cardio-combine", "Circuit cardio combiné", M.CARDIO_CIRCUIT_COMBO, "exact"),
  entry("navette-progressive", "Course navette progressive", M.SPRINT, "exact"),
  entry("gammes-cardio-ballon", "Gammes cardio ballon", M.BALL_DRIBBLE, "approx", { ball: true }),
  entry("hiit-leger-20-10", "HIIT léger 20/10", M.JUMPING_JACKS, "exact"),
  entry("marche-active-accelerations", "Marche active + accélérations courtes", M.JOG, "approx"),
  entry("jonglage-cardio-deplacement", "Jonglage cardio en déplacement", M.JUGGLE_ON_THE_MOVE, "exact", { ball: true }),
  ...
  entry("gainage-lombaire-doux", "Gainage lombaire doux", M.PLANK_FRONT_HOLD, "exact"),
  ...
  entry("auto-massage-quadriceps", "Auto-massage quadriceps", M.QUAD_FOAM_ROLL, "exact"),
  ...
  entry("relance-pied-courte-longue", "Relance au pied courte/longue", M.PASS_STRIKE, "exact", { gk: true, ball: true }),
  entry("reflexes-mains-rapprochees", "Réflexes mains rapprochées", M.GK_REFLEX_SAVE, "exact", { gk: true }),
  entry("sorties-aeriennes", "Sorties aériennes", M.GK_HIGH_CATCH, "exact", { gk: true, ball: true }),
  entry("blocage-sol-parade-basse", "Blocage au sol (parade basse)", M.GK_LOW_BLOCK_MOVE, "exact", { gk: true }),
  entry("duel-face-a-face-gardien", "Duel face à face (1v1 gardien)", M.GK_ENGAGE_1V1, "exact", { gk: true }),
  ...
  entry("enchainement-plongeon-releve-plongeon", "Enchaînement plongeon-relevé-plongeon", M.GK_LATERAL_DIVE, "exact", { gk: true }),
  entry("footwork-echelle-gardien", "Footwork gardien (échelle)", M.HIGH_KNEES, "exact", { gk: true }),
  entry("reduction-angle-1v1", "Réduction d'angle en 1 contre 1", M.GK_NARROW_ANGLE, "exact", { gk: true }),
  entry("reflexes-frappes-rapprochees", "Réflexes sur frappes rapprochées", M.GK_REFLEX_SAVE, "exact", { gk: true }),
  entry("distribution-sous-pression", "Distribution sous pression temporelle", M.GK_DISTRIBUTION, "exact", { gk: true }),
];
